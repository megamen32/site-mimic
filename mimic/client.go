...
type transportOptions struct {
	...
	serverName  string
	ttl         int
	// resolve pins DNS: host → dial IP. The ClientHello SNI keeps the
	// original host, so CDN/vhost routing still lands correctly. Needed
	// for sites whose edge hosts have no public A records: the CDN edge
	// IP serves them by SNI alone.
...
// WithResolve pins DNS: host → dial IP. The ClientHello SNI keeps the
// original host, so CDN/vhost routing still lands correctly. Needed for
// sites whose edge hosts are hidden from public DNS.
func WithResolve(resolve map[string]string) Option {
	return func(o *transportOptions) { o.resolve = resolve }
}
...
func New(p Profile, opts ...Option) (*http.Client, error) {
	...
	dial := func(ctx context.Context, network, addr string, requireH2 bool) (net.Conn, error) {
		...
		dialAddr := addr
		if ip, ok := o.resolve[host]; ok {
			dialAddr = net.JoinHostPort(ip, portOf(addr))
		}
		rawConn, err := dialUnderProxy(ctx, dialer, o.proxyURL, network, dialAddr)
		if err != nil {
			return nil, err
		}
		sni := host
...
// portOf returns the port part of a host:port address, defaulting to 443.
