...
| 4 | Windows 11 23H2 (198.51.100.20) | Chrome | 151.0.7922.174 | **headed, AdGuard ENABLED** (AdguardSvc + browser extension; TLS layer untouched by it — JA4 stayed genuine-Chrome) | `t13d1516h2_8daaf6152771_806a8c22fdea` (+ per-connection 17-ext `t13d1517h2_8daaf6152771_a87ad97598a9`) | **128** | UA `Windows NT 10.0; Win64; x64 … Chrome/151.0.0.0`, sec-ch-ua `"Not=A?Brand";v="99", "Google Chrome";v="151", "Chromium";v="151"`, sec-ch-ua-platform `"Windows"`, `gzip, deflate, br, zstd`, `Priority: u=0, i` → `examples/vk-ru-windows` |
...
| 7b | **macOS 15.7.8** (Mac mini 2012 x86_64, 198.51.100.24) | curl (LibreSSL) | 8.7.1 | native macOS TLS stack | `t13i4906h2_0d8feac7bc37_7395dae3b2f3` (49 ciphers, 6 exts, h2) | 64 | instantly distinguishable from every browser |
...
- **TTL is the cheapest OS tell**: Windows 128, Linux/Android 64 — and
  site-mimic forges it (`WithTTL` / `ip_ttl`). A Linux host can also be
  pinned to the Windows L3/L4 natively: `/etc/sysctl.d/60-win-tcp-mimic.conf`
  (TTL 128, no-TS, wscale 8) via `tools/win-tcp-preset.sh apply` — measured
  TTL on the wire is 127 after one hairpin hop, no per-connection overrides.
  macOS hosts stay native (TTL 64, BSD SYN options) — a distinct, honest
  fingerprint.
