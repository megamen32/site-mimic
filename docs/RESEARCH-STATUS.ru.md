...
Статус: НЕ реализован. Транспортная мимикрия (TLS/h2/QUIC/TTL/TCP) его не
покрывает: антибот смотрит и на «как ведёт себя клиент после навигации».

План (по порядку ценности):
1. **Профиль ресурсного графа**: в `profile.json` — `resource_plan`:
   последовательность запросов (path, resource-type, referer, delay_ms с
   логнормальным джиттером), воспроизводящая порядок главного документа,
   статики и XHR — как у реального браузера.
2. **Sec-Fetch/referer-цепочки**: наследование Sec-Fetch-Site/Referer по
   графу из п.1 (навигация → same-origin подресурсы → XHR).
...
4. **Тайминги**: пер-запросные задержки из реального HAR-эталона (снять HAR
   с реального Chrome через DevTools и подмешать в resource_plan).
5. **Наборы заголовков по типу ресурса**: отдельные header-наборы для
   image/XHR/font запросов (сейчас профили описывают только навигацию).

Оценка: п.1-2 — ближайший шаг после стабилизации транспорта; п.3 — малый;
п.4-5 — по потребности конкретного целевого сайта.
