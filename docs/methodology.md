...
| HTTP/2 frames | SETTINGS/WINDOW_UPDATE/PRIORITY values and order (Akamai fp) | **Go default — gap** (roadmap) |
| HTTP headers | names, values, order | **values matched**; order is Go's — gap (roadmap) |
...
- **HTTP/2 frame fingerprint.** `x/net/http2` sends its own SETTINGS
  ...
  of 1 GiB…) in its own order. Browser-shaped h2 framing needs a custom
  framer — the main roadmap item.
- **Header wire order.** Browsers send headers in a fixed per-version order;
  Go canonicalizes and reorders. Values and the set are exact.
- **accept-encoding.** Profiles deliberately omit it: Go decompresses
  transparently only when it owns the header, and cannot decode Brotli.
...
- **h2-only sites.** HTTPS is served via `http2.Transport`; a host that
  refuses h2 is remembered and retried over HTTP/1.1 with the same uTLS
  dial.
