...
// verification stand (fp.example.test).
//
// haproxy routes SNI fp.example.test here with send-proxy-v2, so fpd
...
func main() {
	...
	certFile := flag.String("cert", "/etc/letsencrypt/live/fp.example.test/fullchain.pem", "TLS certificate")
	keyFile := flag.String("key", "/etc/letsencrypt/live/fp.example.test/privkey.pem", "TLS key")
...
func runSelftest(addr string) {
	clientIP, clientPort := "198.51.100.10", 61234
	...
	req, _ := http.NewRequest("GET", "https://fp.example.test/fp?format=json", nil)
